...
export default function Drums() {
  ...
  useEffect(() => {
    const loadPageData = async () => {
      try {
        ...
        const response = await fetchCategoryContents("드럼");
        setAuthorNicknames(response.authorNicknames);
        setContents(response.contents);
      } catch (error) {
        console.error("드럼 목록 로딩 실패", error);
      }
    };
  ...
  const sanctionContent = async (contentId: string, reason: string) => {
    await sanctionContentById(contentId, reason);
    const response = await fetchCategoryContents("드럼");
    setAuthorNicknames(response.authorNicknames);
    setContents(response.contents);
    setOpenSanctionMenuId(null);
    ...
    const unsanctionContent = async (contentId: string) => {
      await unsanctionContentById(contentId);
      const response = await fetchCategoryContents("드럼");
      setAuthorNicknames(response.authorNicknames);
      setContents(response.contents);
      setOpenSanctionMenuId(null);
  ...
  return (
    <main className="min-h-screen bg-background text-foreground pt-32">
      ...
      >
        ...
        <h1 className="text-4xl font-display font-bold mb-8 text-white">드럼 (DRUMS)</h1>
                ...
                <div className="relative w-full h-48 bg-background overflow-hidden">
                  <img
                    ...
                    className={`w-full h-full object-cover transition-transform duration-300 hover:scale-110 ${
                      content.isSanctioned ? "grayscale brightness-50" : ""
                    }`}
                  />
